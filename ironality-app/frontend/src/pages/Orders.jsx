...
export default function Orders() {
  ...
  try {
    ...
    const res = await axios.post(
      ...
      {
        headers: { Authorization: `Bearer ${token}` },
        validateStatus: () => true, // always resolve, we want to inspect response
      }
    );
    ...
    if (!res.data) {
      alert("Payment failed: Response is empty.");
      return;
    }
    ...
    if (!res.data.url) {
      alert("Payment failed: Stripe URL missing. Check console.");
      console.error("❌ Missing Stripe URL. Full response:", res);
      return;
    }

    console.log("✅ Redirecting to Stripe checkout URL:", res.data.url);
